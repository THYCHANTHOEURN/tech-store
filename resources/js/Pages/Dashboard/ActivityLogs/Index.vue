    ...
    <DashboardLayout>
        ...
        <v-container fluid class="py-8">
            ...
            <FilterBar :loading="loading" :total-items="activities.total" items-label="activities"
                ...
                <template #filters>
                    <v-col cols="12" md="3">
                        <SearchField v-model="causer" label="Causer Name" :loading="loading" @search="applyFilters" @clear="applyFilters" />
                    </v-col>
                    <v-col cols="12" md="3">
                        <SearchField v-model="description" label="Description" :loading="loading" @search="applyFilters" @clear="applyFilters" />
                    </v-col>
                    <v-col cols="12" md="3">
                        <v-text-field v-model="dateFrom" label="Date From" type="date" variant="outlined" density="comfortable" @change="applyFilters" />
                    </v-col>
                    <v-col cols="12" md="3">
                        <v-text-field v-model="dateTo" label="Date To" type="date" variant="outlined" density="comfortable" @change="applyFilters" />
                    </v-col>
                </template>
            </FilterBar>
            ...
            <v-row>
                <v-col cols="12">
                    <v-card>
                        <v-data-table :headers="headers" :items="activities.data" :loading="loading" class="elevation-0" hide-default-footer>
                        ...
                        <div class="d-flex justify-center py-4">
                            <v-pagination v-if="activities.last_page" v-model="currentPage" :length="activities.last_page"
                                total-visible="7" @update:model-value="changePage" rounded></v-pagination>
                            <v-select v-model="perPage" :items="perPageOptions" label="Rows per page" class="ml-4" style="max-width: 120px;" @update:model-value="changePerPage" hide-details></v-select>
                        </div>
                    </v-card>
                </v-col>
            </v-row>
